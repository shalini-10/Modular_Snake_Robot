...
class SnakeGaits(object):
	...
	def set_configuration(self,h1,v1,h2,v2,pause):
		...
		pwm.set_pwm_freq(60)
		pwm.set_pwm(m1,0,self.angle_calc(h1))
		pwm.set_pwm(m2,0,self.angle_calc(v1))
		pwm.set_pwm(m3,0,self.angle_calc(h2))
		pwm.set_pwm(m4,0,self.angle_calc(v2))
		time.sleep(pause)
	...
	#Foward gait
	def foward_gait(self,Amp,loops,loop_pause):
		#Amp: amplitude
		#loop_pause = 0.001
		for j in range(0,loops+1):
			for i in range(0,100):
				...
				v1=Amp*(i/100.0)
				v2=-theta-(math.asin((166.26/188.23)*math.sin(theta*(math.pi/180.0))))*(180.0/math.pi)
				h1=0
				h2=0
	...
	def backward_gait(self,Amp,loops,loop_pause):
		...
		for i in range(0,loops+1):
			for j in range(0,100):
				v2=Amp*(j/100.0)
				v1=-theta-(math.asin((166.26/188.23)*math.sin(theta*(math.pi/180.0))))*(180.0/math.pi)
				h1=0
				h2=0
				self.set_configuration(h1,v1,h2,v2,0.0001)
			time.sleep(0.001)
